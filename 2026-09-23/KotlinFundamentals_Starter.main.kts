...
fun describe(result: TransferResult) : String {
    when (result) {
        is TransferResult.Success -> return "Success. Confirmation ID: ${result.confirmationId}"
        is TransferResult.Failure -> return "Failure. Reason: ${result.reason}"
    }
}

println(describe(TransferResult.Success("2314")))
println(describe(TransferResult.Failure("There was an error")))
...
// Mistakes I made for self reference: Using colons instead of = for arguments. Colons are ONLY used for declaring variable types
// Forgot to wrap ID in double quotations, and it did not convert to a string
// Forgot to add .0 to the balance and the compiler would not automatically convert them to doubles
suspend fun fetchAccounts(): List<Account> {
    delay(500)
    val account1 = Account(id = "1", name = "John", balance = 9034.0)
    val account2 = Account(id = "2", name = "George", balance = -34.0)
    val account3 = Account(id = "3", name = "Julianne", balance = 342.0)
...
fun main() = runBlocking {
    ...
    // operations here.
    val accountsWithNegativeBalance = accounts.filter { it.balance < 0}
    val totalBalance = accounts.sumOf { it.balance }
    
    println(accountsWithNegativeBalance)
    println(totalBalance.asCurrency())
}
