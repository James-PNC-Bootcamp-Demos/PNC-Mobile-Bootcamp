...
let t1 = Transaction(id: "1", date: Date(), amount: 5, description: "Protein bar", isDebit: true)
let t2 = Transaction(id: "2", date: Date(), amount: 25, description: "Protein shake", isDebit: true)
t1.printDetails()
...
let transactionArray = [t1, t2]

printAll(items: transactionArray)
...
class AccountDashboard {
    ...
    func showSummary(for accountId: String) {
        print("Account [\(accountId)]: Balance $\(dataSource.fetchBalance(for: accountId))")
    }
}
...
func checkNil(user: UserProfile){
    let available = user.address?.zip ?? "No Zip available"
    print(available)
}
...
func transfer(from sourceId: String?, to destId: String?, amount: Double?) {
    if let sourceId, let destId, let amount, amount > 0 {
        print("Transfer $\(amount) from \(sourceId) to \(destId) approved")
    } else {
...
// I would never force unwrap user input because it could be any kind of string, null included. I would unwrap it using if let instead to handle a potential null value.
...
enum TransferError : LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .insufficientFunds(let available): return "Insufficient funds. Available funds: \(available)"
        case .accountNotFound(let id): return "Account with id \(id) not found."
        case .dailyLimitExceeded(let limit, let attempted): return "Daily limit exceeded. Limit: \(limit). Attempted: \(attempted)"
...
func executeTransfer(amount: Double, fromBalance: Double, toAccountId: String,
    ...
    guard dailyLimit > amount + dailyUsed else {
        throw TransferError.dailyLimitExceeded(limit: dailyLimit, attempted: amount + dailyUsed)
    }
